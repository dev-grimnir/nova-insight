class NovaDashboardController {
    ...
    showAdminManager() {
        ...
        this.#adminManagerController = new NovaAdminManagerController(this);
        this.#adminManagerController.show();
    }

    showAddCustomer() {
        const addController = new NovaAddCustomerController(this.#tabController);
        addController.show();
    }
    
    static async create() {
        const model = new NovaDashboardModel();
        const controller = new NovaDashboardController(model);
        const tabController = new NovaTabController(controller);  
        controller.#tabController = tabController;
        const view = new NovaDashboardView(controller);  // createElements calls mountTabView here
    ...
    mountTabView(containerEl) {
        this.#tabController.view = new NovaTabView(this.#tabController);
    ...
    #attachModalListeners() {
        ...
        document.addEventListener('nova:modal-opened', () => {
            this.#modalActive = true;
        });

        document.addEventListener('nova:modal-closed', () => {
            this.#modalActive = false;
        });
    }
     ...
     * 
     * Now completely delegates crypto setup to NovaCryptoController.
    ...
    async initAsync() {
        ...
        await NovaCryptoController.initMasterKey();

        if (!NovaCryptoController.hasMasterKey) {
            this.passphraseController = new NovaPassphraseController(this);
            const accepted = await this.passphraseController.show();
            if (!accepted) {
                NovaToast.error("A passphrase is required. Reload the page to try again.");
                return;
            }
        }
        ...
        try {
            ...
            try {
                if (customer.eventHistory.length > 0) {
                    ...
                    const newEvents = await NovaHTTPController.paginateReportLogs(
                    ...
                    for (const lookbackMs of lookbackPeriods) {
                        ...
                        const events = await NovaHTTPController.paginateReportLogs(
            ...
            for (const trySince of lookbackPeriods) {
                latest = await NovaHTTPController.getLatestEntry(customer.radiusUsername, trySince);
