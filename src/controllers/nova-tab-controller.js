class NovaTabController {
    ...
    async add(radiusUsername, friendlyName) {
        ...
        if (activeTab.customers.find(c => c.radiusUsername === trimmed)) {
            NovaToast.error("This customer has already beeen added to this tab.");
            return;
        }
    
        const ctrl = new NovaCustomerController(this.dashboardController, trimmed, friendlyName);
        ...
        try {
            const since = new Date(Date.now() - NovaCustomerModel.RETENTION_MS);
        ...
        try {
            ...
            if (ctrl.model.status === 'Account Not Found') {
                this.remove(trimmed);
                NovaToast.error('Customer not found in RADIUS (' + trimmed + ')');
                return;
            }
    ...
    initDefaultTab() {
        const defaultTab = new NovaTabModel('All', true);
    ...
    async addTab(label) {
        const tab = new NovaTabModel(label);
        this.tabs.push(tab);
        await this.save();
        this.view.render();
        this.dashboardController.view.renderTabBar();
    ...
    async save() {
        try {
            ...
            console.error('[NovaTabController.save]', e);
        }
    }
    ...
    async load() {
        ...
        try {
            const json = JSON.parse(await NovaCryptoController.decryptData(data));
            this.tabs = json.tabs.map(t => NovaTabModel.fromJSON(t, this.dashboardController));
            this.view.render();
        } catch (e) {
            console.error('[NovaTabController.load]', e);
            this.initDefaultTab();
        }
    }
    ...
    async moveCustomerToTab(radiusUsername, targetLabel) {
        ...
        if (targetTab.customers.some(c => c.radiusUsername === radiusUsername)) {
            ...
            NovaToast.error(`"${label}" is already in "${targetLabel}"`);
            return;
        }
