class NovaCustomerView extends BaseNovaView {
    ...
    constructor(controller) {
        ...
        if (!document.getElementById('nova-drag-handle-style')) {
            const style = document.createElement('style');
            style.id = 'nova-drag-handle-style';
            style.textContent = `
                .drag-handle {
    ...
    #attachDragListeners() {
        this.#tr.addEventListener('dragstart', (e) => {
            ...
            e.dataTransfer.setData('application/x-nova-customer', this.#controller.radiusUsername);
            this.#tr.classList.add('nova-row-dragging');
            const tabCtrl = this.#controller.dashboardController?.getTabController?.();
            tabCtrl?.beginDrag();
        });
    
        this.#tr.addEventListener('dragend', () => {
            this.#tr.classList.remove('nova-row-dragging');
            const tabCtrl = this.#controller.dashboardController?.getTabController?.();
            tabCtrl?.endDrag();
        });
    }
    ...
    #renderContent() {
        ...
        const inlineSnapshot = new NovaInlineSnapshotView(cust).render();
