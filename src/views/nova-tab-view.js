class NovaTabView extends BaseNovaView {
    ...
    #injectStyles() {
        if (document.getElementById('nova-row-drag-style')) return;
        const style = document.createElement('style');
        style.id = 'nova-row-drag-style';
        style.textContent = `
            .nova-row-drop-indicator {
                position: fixed;
                height: 3px;
                background: #34d399;
                pointer-events: none;
    ...
    #setupDropIndicator() {
        ...
        this.dropIndicator.className = 'nova-row-drop-indicator';
    ...
    applyPrivacyBlur(enabled) {
        if (this.container) {
            this.container.classList.toggle('nova-privacy-mode', enabled);
        }
    }
    ...
    #buildTable(customers) {
        ...
        table.className = 'nova-customer-table';
