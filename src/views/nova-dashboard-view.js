class NovaDashboardView extends BaseNovaView {
    ...
    static buildColGroupHTML() {
        return '<colgroup>' +
            NovaDashboardView.COLUMNS.map(c => `<col style="width: ${c.width}%;">`).join('') +
            '</colgroup>';
    }
        ...
        return `
            <div class="flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-zinc-900 shrink-0 relative z-10 dashboard-header">
                <div class="flex items-center gap-4">
                    <img src="https://raw.githubusercontent.com/dev-grimnir/nova-insight/main/src/assets/nova-subscriber-logo.png" 
    ...
    renderTableHeader() {
        ...
        temp.innerHTML = NovaDashboardView.buildTheadHTML(activeTab);
    ...
    createElements() {
        ...
        this.panel.classList.add('nova-dashboard');
                    ...
                    <div class="flex-1 bg-zinc-900 border border-zinc-700 rounded-3xl overflow-hidden flex flex-col">
                        ...
                        <div class="px-6 py-1 bg-zinc-900 border-b border-zinc-800">
                            <table class="w-full table-fixed">
                                ${NovaDashboardView.buildColGroupHTML()}
                                ${NovaDashboardView.buildTheadHTML(this.controller.getTabController().getActiveTab())}
                            </table>
                        </div>
                        
                        <!-- Scrollable body — owned by NovaTabView -->
                        <div class="flex-1 overflow-y-auto px-6 nova-scroll">
                            <table class="w-full table-fixed">
                                ${NovaDashboardView.buildColGroupHTML()}
        ...
        if (!document.getElementById('nova-scroll-style')) {
            const style = document.createElement('style');
            style.id = 'nova-scroll-style';
            style.innerHTML = `
                ...
                .nova-dashboard.minimized .poll-slider-tooltip {
                    top: auto !important;
                    bottom: 100% !important;
                    margin-top: 0 !important;
                    margin-bottom: 12px !important;
                }
                .nova-dashboard.minimized .poll-slider-tooltip > div:last-child {
                    top: auto !important;
                ...
                /* Privacy mode blur */
                .nova-privacy-mode td:nth-child(1),
                .nova-privacy-mode td:nth-child(2) {
                    filter: blur(5px);
                    transition: filter 300ms ease;
                }
                .nova-privacy-mode tr:hover td:nth-child(1),
                .nova-privacy-mode tr:hover td:nth-child(2) {
                    filter: blur(0);
                }
    
                /* Scrollbar */
                .nova-scroll::-webkit-scrollbar { width: 7px; }
                .nova-scroll::-webkit-scrollbar-track { background: #18181b; border-radius: 9999px; }
                .nova-scroll::-webkit-scrollbar-thumb { background: #34d399; border-radius: 9999px; border: 2px solid #18181b; }
                .nova-scroll::-webkit-scrollbar-thumb:hover { background: #10b981; }
                .nova-scroll { scrollbar-width: thin; scrollbar-color: #34d399 #18181b; }
                ...
                .nova-tab-btn .tab-close {
                    margin-left: 8px;
                    font-size: 14px;
                    color: #ffffff;
                    transition: color 150ms;
                }
                .nova-tab-btn .tab-close:hover {
                    color: #ef4444;
                }
                .nova-tab-add {
                    padding: 4px 10px;
                ...
                .nova-tab-add:hover { color: #34d399; }
                ...
                .nova-tab-btn[draggable="true"] {
                    cursor: grab;
                }
                .nova-tab-btn.dragging {
                    opacity: 0.4;
                    transform: scale(1.02);
                }
                .nova-tab-btn.drop-before::before,
                .nova-tab-btn.drop-after::after {
                    content: '';
                    position: absolute;
                    top: 4px;
                    bottom: 0;
                ...
                .nova-tab-btn.drop-before::before { left: -2px; }
                .nova-tab-btn.drop-after::after { right: -2px; }

                /* Mode toggle icon (sort vs manual) */
                .nova-tab-btn .tab-mode-toggle {
                    margin-right: 6px;
                    cursor: pointer;
                    color: #a1a1aa;
                    display: inline-flex;
                ...
                .nova-tab-btn .tab-mode-toggle:hover {
                    color: #34d399;
                }

                /* Customer-drop target (whole tab highlights, not just a side bar) */
                .nova-tab-btn.customer-drop-target {
        ...
        this.escListener = (e) => {
            ...
            if (document.querySelector('.nova-modal, #add-customer-modal, #passphrase-modal, [id*="modal"]')) return;
            e.preventDefault();
            this.toggleMinimize();
        };
    ...
    renderTabBar() {
        ...
        tabs.forEach((tab, idx) => {
            ...
            btn.className = `nova-tab-btn${tab.isActive ? ' active' : ''}`;
            ...
            btn.addEventListener('dragend', () => {
                ...
                this.tabBar.querySelectorAll('.nova-tab-btn').forEach(b => {
            ...
            btn.addEventListener('dragover', (e) => {
                ...
                if (e.dataTransfer.types.includes('application/x-nova-customer')) return;
            ...
            btn.addEventListener('drop', (e) => {
                ...
                e.stopPropagation();
                btn.classList.remove('customer-drop-target');
                const username = e.dataTransfer.getData('application/x-nova-customer');
                if (!username) return;
    ...
    applyPrivacyBlur() {
        ...
        if (tbody) {
            tbody.classList.toggle('nova-privacy-mode', this.controller.model.settings.privacyEnabled);
        }
    }
